...
# this package
from dulwich.objects import S_ISGITLINK, TreeEntry

__all__ = ["TreeChange", "walk_trees", "tree_changes", "tree_changes_for_merge", "RenameDetector"]
...
def tree_changes(
	...
	if (rename_detector is not None and tree1_id is not None and tree2_id is not None):
		for change in rename_detector.changes_with_renames(
				tree1_id, tree2_id, want_unchanged=want_unchanged, include_trees=include_trees
				):
			yield change
		return
...
def tree_changes_for_merge(store, parent_tree_ids, tree_id, rename_detector=None):
    ...
    """
	all_parent_changes = [
			tree_changes(store, t, tree_id, rename_detector=rename_detector) for t in parent_tree_ids
			]
...
class RenameDetector(object):
	...
	def _collect_changes(self, tree1_id, tree2_id):
		...
		for change in tree_changes(
				self._store, tree1_id, tree2_id, want_unchanged=want_unchanged, include_trees=self._include_trees
				):
...
try:
	...
	# this package
	from dulwich._diff_tree import _count_blocks, _is_tree, _merge_entries  # type: ignore
except ImportError:
	pass
