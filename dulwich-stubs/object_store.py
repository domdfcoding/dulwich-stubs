...
class BaseObjectStore(object):
	...
	def determine_wants_all(self, refs):
		return [
				sha for (ref, sha) in refs.items()
				if sha not in self and not ref.endswith(ANNOTATED_TAG_SUFFIX) and not sha == ZERO_SHA
				]
	...
	def add_pack_data(self, count, pack_data, progress=None):
		...
		try:
			write_pack_data(f, count, pack_data, progress, compression_level=self.pack_compression_level)
	...
	def tree_changes(
		...
		for change in tree_changes(
			...
			yield ((change.old.path, change.new.path), (change.old.mode, change.new.mode),
	...
	def find_missing_objects(
        ...
        """
		finder = MissingObjectFinder(self, haves, wants, shallow, progress, get_tagged, get_parents=get_parents)
	...
	def generate_pack_data(self, have, want, shallow=None, progress=None, ofs_delta=True):
		...
		return pack_objects_to_data(self.generate_pack_contents(have, want, shallow, progress))
	...
	def _collect_ancestors(self, heads, common=set(), shallow=set(), get_parents=lambda commit: commit.parents):
	...
	def _complete_thin_pack(self, f, path, copier, indexer):
		...
		for ext_sha in indexer.ext_refs():
			...
			crc32 = write_pack_object(
					f, type_num, data, sha=new_sha, compression_level=self.pack_compression_level
					)
	...
	for e in lst:
		try:
			...
			if isinstance(o, Commit):
				...
				tags.add(e)
				tagged = o.object[1]
				c, t, o = _split_commits_and_tags(obj_store, [tagged], ignore_unknown=ignore_unknown)
				commits |= c
				tags |= t
				others |= o
			else:
...
class MissingObjectFinder(object):
	...
	def __init__(
		...
		have_commits, have_tags, have_others = (_split_commits_and_tags(object_store, haves, True))
		want_commits, want_tags, want_others = (_split_commits_and_tags(object_store, wants, False))
	...
	def next(self):
		...
		if not leaf:
			...
			if isinstance(o, Commit):
				...
				self.add_todo([(s, n, not stat.S_ISDIR(m)) for n, m, s in o.iteritems() if not S_ISGITLINK(m)])
			elif isinstance(o, Tag):
