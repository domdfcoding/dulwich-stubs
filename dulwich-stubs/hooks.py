		...
		if len(args) != self.numparam:
			raise HookError(
					"Hook %s executed with wrong number of args. \
                            Expected %d. Saw %d. args: %s" % (self.name, self.numparam, len(args), args)
					)
...
class PostReceiveShellHook(ShellHook):
	...
	def execute(self, client_refs):
		...
		try:
			...
			p = subprocess.Popen(
					self.filepath, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env
					)
