from collections import namedtuple
from dulwich.file import GitFile as GitFile
from dulwich.object_store import BaseObjectStore as BaseObjectStore
from dulwich.objects import Blob as Blob, S_IFGITLINK as S_IFGITLINK, S_ISGITLINK as S_ISGITLINK, Tree as Tree, hex_to_sha as hex_to_sha, sha_to_hex as sha_to_hex
from dulwich.pack import SHA1Reader as SHA1Reader, SHA1Writer as SHA1Writer
from typing import Any, BinaryIO, Callable, Dict, Iterable, Iterator, List, Optional, Tuple

IndexEntry = namedtuple('IndexEntry', ['ctime', 'mtime', 'dev', 'ino', 'mode', 'uid', 'gid', 'size', 'sha', 'flags'])
FLAG_STAGEMASK: int
FLAG_VALID: int
FLAG_EXTENDED: int
DEFAULT_VERSION: int

def pathsplit(path: Any): ...
def pathjoin(*args: Any): ...
def read_cache_time(f: Any): ...
def write_cache_time(f: Any, t: Any) -> None: ...
def read_cache_entry(f: Any): ...
def write_cache_entry(f: Any, entry: Any) -> None: ...
def read_index(f: BinaryIO) -> Any: ...
def read_index_dict(f: Any): ...
def write_index(f: BinaryIO, entries: List[Any], version: Optional[int]=...) -> Any: ...
def write_index_dict(f: BinaryIO, entries: Dict[bytes, IndexEntry], version: Optional[int]=...) -> None: ...
def cleanup_mode(mode: int) -> int: ...

class Index:
    def __init__(self, filename: Any) -> None: ...
    @property
    def path(self): ...
    def write(self) -> None: ...
    def read(self) -> None: ...
    def __len__(self) -> int: ...
    def __getitem__(self, name: bytes) -> IndexEntry: ...
    def __iter__(self) -> Iterator[bytes]: ...
    def get_sha1(self, path: bytes) -> bytes: ...
    def get_mode(self, path: bytes) -> int: ...
    def iterobjects(self) -> Iterable[Tuple[bytes, bytes, int]]: ...
    def iterblobs(self): ...
    def clear(self) -> None: ...
    def __setitem__(self, name: Any, x: Any) -> None: ...
    def __delitem__(self, name: Any) -> None: ...
    def iteritems(self): ...
    def items(self): ...
    def update(self, entries: Any) -> None: ...
    def changes_from_tree(self, object_store: Any, tree: Any, want_unchanged: bool = ...): ...
    def commit(self, object_store: Any): ...

def commit_tree(object_store: BaseObjectStore, blobs: Iterable[Tuple[bytes, bytes, int]]) -> bytes: ...
def commit_index(object_store: BaseObjectStore, index: Index) -> bytes: ...
def changes_from_tree(names: Iterable[bytes], lookup_entry: Callable[[bytes], Tuple[bytes, int]], object_store: BaseObjectStore, tree: Optional[bytes], want_unchanged: Any=...) -> Iterable[Tuple[Tuple[Optional[bytes], Optional[bytes]], Tuple[Optional[int], Optional[int]], Tuple[Optional[bytes], Optional[bytes]]]]: ...
def index_entry_from_stat(stat_val: Any, hex_sha: bytes, flags: int, mode: Optional[int]=...) -> Any: ...
def build_file_from_blob(blob: Any, mode: Any, target_path: Any, honor_filemode: bool = ..., tree_encoding: str = ...): ...

INVALID_DOTNAMES: Any

def validate_path_element_default(element: Any): ...
def validate_path_element_ntfs(element: Any): ...
def validate_path(path: Any, element_validator: Any = ...): ...
def build_index_from_tree(root_path: Any, index_path: Any, object_store: Any, tree_id: Any, honor_filemode: bool = ..., validate_path_element: Any = ...) -> None: ...
def blob_from_path_and_mode(fs_path: Any, mode: Any, tree_encoding: str = ...): ...
def blob_from_path_and_stat(fs_path: Any, st: Any, tree_encoding: str = ...): ...
def read_submodule_head(path: Any): ...
def get_unstaged_changes(index: Index, root_path: Any, filter_blob_callback: Any=...) -> Any: ...

os_sep_bytes: Any

def index_entry_from_path(path: Any, object_store: Optional[Any] = ...): ...
def iter_fresh_entries(paths: Any, root_path: Any, object_store: Optional[BaseObjectStore]=...) -> Any: ...
def iter_fresh_blobs(index: Any, root_path: Any) -> None: ...
def iter_fresh_objects(paths: Any, root_path: Any, include_deleted: bool = ..., object_store: Optional[Any] = ...) -> None: ...
def refresh_index(index: Any, root_path: Any) -> None: ...
