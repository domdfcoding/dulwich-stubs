...
from dulwich.objects import ZERO_SHA, git_line, valid_hexsha
...
class DictRefsContainer(RefsContainer):
	...
	def remove_if_equals(self, name, old_ref, committer=None, timestamp=None, timezone=None, message=None):
		...
		try:
			...
			self._log(
					name, old, None, committer=committer, timestamp=timestamp, timezone=timezone, message=message
					)
		return True
...
def strip_peeled_refs(refs):
	...
	return {ref: sha for (ref, sha) in refs.items() if not ref.endswith(ANNOTATED_TAG_SUFFIX)}
