...
class ConfigDict(Config, MutableMapping):
	...
	def _check_section_and_name(self, section, name):
		...
		section = tuple([
				subsection.encode(self.encoding) if not isinstance(subsection, bytes) else subsection
...
def _format_string(value):
	if (
			value.startswith(b" ") or value.startswith(b"\t") or value.endswith(b" ") or b'#' in value
			or value.endswith(b"\t")
			):
	...
	while i < len(value):
		...
		if c == ord(b"\\"):
			...
			try:
				...
				raise ValueError("escape character in %r at %d before end of string" % (value, i))
			except KeyError:
				raise ValueError(
