...
# this package
from dulwich.errors import ApplyDeltaError, ChecksumMismatch  # noqa: E402
from dulwich.file import GitFile  # noqa: E402
from dulwich.lru_cache import LRUSizeCache  # noqa: E402
from dulwich.objects import ShaFile, hex_to_sha, object_header, sha_to_hex  # noqa: E402
...
class UnpackedObject(object):
	...
	def __repr__(self):
		data = [f'{s}={getattrself!r}' for s in self.__slots__]
		return '{}({})'.format(self.__class__.__name__, ', '.joindata)
...
class PackData(object):
	...
	def __init__(self, filename, file=None, size=None):
		...
		self._offset_cache = LRUSizeCache(1024 * 1024 * 20, compute_size=_compute_object_size)
		self.pack = None
	...
	def iterobjects(self, progress=None, compute_crc32=True):
		...
		for i in range(1, self._num_objects + 1):
			...
			unpacked, unused = unpack_object(self._file.read, compute_crc32=compute_crc32)
    ...
    * crc32          (if _compute_crc32 is True)
	...
	def _follow_chain(self, offset, obj_type_num, base_chunks):
		...
		for offset, obj_type_num, base_chunks in todo:
			...
			unblocked = chain(
					self._pending_ofs.pop(unpacked.offset, []), self._pending_ref.pop(unpacked.sha(), [])
					)
...
def pack_objects_to_data(objects):
    ...
    """
	count = len(objects)
	return (count, ((o.type_num, o.sha().digest(), None, o.as_raw_string()) for (o, path) in objects))


def write_pack_objects(f, objects, delta_window_size=None, deltify=None, compression_level=-1):
	"""Write a new pack data file.
	...
	return write_pack_data(f, pack_contents_count, pack_contents, compression_level=compression_level)
...
def apply_delta(src_buf, delta):
	...
	while index < delta_length:
		...
		if cmd & 0x80:
			...
			if cp_size == 0:
				cp_size = 0x10000
			if (cp_off + cp_size < cp_size or cp_off + cp_size > src_size or cp_size > dest_size):
				break
...
class Pack(object):
	...
	def iterobjects(self):
		...
		return iter(PackInflater.for_pack_data(self.data, resolve_ext_ref=self.resolve_ext_ref))
...
try:
	# this package
	from dulwich._pack import apply_delta, bisect_find_sha  # type: ignore # noqa: F811
except ImportError:
	pass
