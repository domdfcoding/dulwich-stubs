...
IndexEntry = collections.namedtuple(
		'IndexEntry', ['ctime', 'mtime', 'dev', 'ino', 'mode', 'uid', 'gid', 'size', 'sha', 'flags']
		)
...
def write_index_dict(f: BinaryIO, entries: Dict[bytes, IndexEntry], version: Optional[int] = None) -> None:
...
class Index(object):
	...
	def changes_from_tree(self, object_store, tree, want_unchanged=False):
		...
		for (name, mode, sha) in changes_from_tree(
				self._byname.keys(), lookup_entry, object_store, tree, want_unchanged=want_unchanged
				):
			yield (name, mode, sha)
...
def changes_from_tree(
		...
		) -> Iterable[Tuple[Tuple[Optional[bytes], Optional[bytes]],
...
def index_entry_from_stat(stat_val, hex_sha: bytes, flags: int, mode: Optional[int] = None):
...
def build_file_from_blob(blob, mode, target_path, honor_filemode=True, tree_encoding='utf-8'):
