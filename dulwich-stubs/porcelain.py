...
from dulwich.index import blob_from_path_and_stat, get_unstaged_changes
from dulwich.object_store import tree_lookup_path
from dulwich.objects import Commit, Tag, format_timezone, parse_timezone, pretty_format_tree_entry
from dulwich.objectspec import parse_commit, parse_object, parse_ref, parse_reftuples, parse_tree
from dulwich.pack import write_pack_index, write_pack_objects
from dulwich.patch import write_tree_diff
from dulwich.protocol import ZERO_SHA, Protocol
from dulwich.refs import ANNOTATED_TAG_SUFFIX, LOCAL_BRANCH_PREFIX, RefsContainer, strip_peeled_refs
from dulwich.repo import BaseRepo, Repo
from dulwich.server import FileSystemBackend, ReceivePackHandler, TCPGitServer, UploadPackHandler
...
def path_to_tree_path(repopath, path, tree_encoding=DEFAULT_ENCODING):
	...
	if sys.version_info < (3, 6):
		...
		if treepath.startswith(b'..'):
			err_msg = f'Path {path!r} not in repo path ({repopath!r})'
...
def commit(repo=".", message=None, author=None, committer=None, encoding=None):
	...
	with open_repo_closing(repo) as r:
		return r.do_commit(message=message, author=author, committer=committer, encoding=encoding)
...
def clone(
	...
	if outstream is not None:
		# stdlib
		import warnings
		warnings.warn("outstream= has been deprecated in favour of errstream=.", DeprecationWarning, stacklevel=3)
		errstream = outstream
...
def add(repo=".", paths=None):
	...
	with open_repo_closing(repo) as r:
		...
		if not paths:
			paths = list(get_untracked_paths(str(Path(os.getcwd()).resolve()), str(repo_path), r.open_index()))
		relpaths = []
			...
			if not cached:
				try:
					...
					try:
						...
						try:
							committed_sha = tree_lookup_path(r.__getitem__, r[r.head()].tree, tree_path)[1]
						except KeyError:
							committed_sha = None

						if blob.id != index_sha and index_sha != committed_sha:
							raise Error('file has staged content differing ' 'from both the file and head: %s' % p)
	...
	for change in changes:
		...
		if change.type == CHANGE_ADD:
			...
			if change.type == CHANGE_RENAME:
				...
				kind = 'C'
		yield f'{kind:<8}{path1:<20}{path2:<20}'
...
def push(
	...
	with open_repo_closing(repo) as r:
		...
		client, path = get_transport_and_path(remote_location, config=r.get_config_stack(), **kwargs)
		...
		try:
			result = client.send_pack(
					path, update_refs, generate_pack_data=r.generate_pack_data, progress=errstream.write
					)
		except SendPackError as e:
			raise Error("Push to " + remote_location + " failed -> " + e.args[0].decode(), inner=e)
		else:
		...
		for ref, error in (result.ref_status or {}).items():
			if error is not None:
				errstream.write(b"Push of ref %s failed: %s\n" % (ref, error.encode(err_encoding)))
			else:
...
def pull(
	...
	with open_repo_closing(repo) as r:
		...
		def determine_wants(remote_refs):
			selected_refs.extend(parse_reftuples(remote_refs, r.refs, refspecs, force=force))
			return [
					remote_refs[lh]
					for (lh, rh, force_ref) in selected_refs
		...
		client, path = get_transport_and_path(remote_location, config=r.get_config_stack(), **kwargs)
		fetch_result = client.fetch(path, r, progress=errstream.write, determine_wants=determine_wants)
		for (lh, rh, force_ref) in selected_refs:
			try:
...
def status(repo=".", ignored=False):
	...
	with open_repo_closing(repo) as r:
		...
		if ignored:
			...
			untracked_changes = [p for p in untracked_paths if not ignore_manager.is_ignored(p)]
...
def web_daemon(path=".", address=None, port=None):
    ...
    """
	# this package
	from dulwich.web import WSGIRequestHandlerLogger, WSGIServerLogger, make_server, make_wsgi_chain
...
def fetch(
	...
	with open_repo_closing(repo) as r:
		...
		if remote_name is not None:
			_import_remote_refs(
					r.refs, remote_name, fetch_result.refs, message, prune=prune, prune_tags=prune_tags
					)
	return fetch_result
			...
			for tag in sorted_tags:
				...
				if commit_id == tag_commit:
					if commit_count == 0:
						...
						return '{}-{}-g{}'.format(tag_name, commit_count, latest_commit.id.decode('ascii')[:7])
