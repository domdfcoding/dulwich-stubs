...
__all__ = ["InvalidWants", "HTTPUnauthorized", "ReportStatusParser", "read_pkt_refs", "FetchPackResult", "SendPackResult", "GitClient", "check_wants", "TraditionalGitClient", "TCPGitClient", "SubprocessWrapper", "find_git_command", "SubprocessGitClient", "LocalGitClient", "SSHVendor", "StrangeHostname", "SubprocessSSHVendor", "PLinkSSHVendor", "ParamikoSSHVendor", "SSHGitClient", "default_user_agent_string", "default_urllib3_manager", "HttpGitClient", "get_transport_and_path_from_url", "parse_rsync_url", "get_transport_and_path", "get_credentials_from_store"]
...
class HttpGitClient(GitClient):
    ...
    def from_parsedurl(cls, parsedurl, **kwargs):
        ...
        if parsedurl.port:
            netloc = f"{netloc}:{parsedurl.port}"
        if parsedurl.username:
            netloc = f"{parsedurl.username}@{netloc}"
        parsedurl = parsedurl._replace(netloc=netloc)
        return cls(urlunparse(parsedurl), **kwargs)

    def __repr__(self):
        return "{}({!r}, dumb={!r})".format(
            typeself.__name__, self._base_url, self.dumb)
    ...
    def _discover_references(self, service, base_url):
        ...
        if resp.redirect_location:
            ...
            if not resp.redirect_location.endswith(tail):
                raise GitProtocolError(
                        "Redirected from URL {} to URL {} without {}".format(
