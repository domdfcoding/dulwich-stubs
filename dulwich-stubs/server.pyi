...
from dulwich import log_utils as log_utils
from dulwich.archive import tar_stream as tar_stream
from dulwich.errors import ApplyDeltaError as ApplyDeltaError, ChecksumMismatch as ChecksumMismatch, GitProtocolError as GitProtocolError, HookError as HookError, NotGitRepository as NotGitRepository, ObjectFormatException as ObjectFormatException, UnexpectedCommandError as UnexpectedCommandError
from dulwich.objects import Commit as Commit, valid_hexsha as valid_hexsha
from dulwich.pack import write_pack_objects as write_pack_objects
from dulwich.protocol import BufferedPktLineWriter as BufferedPktLineWriter, CAPABILITIES_REF as CAPABILITIES_REF, CAPABILITY_AGENT as CAPABILITY_AGENT, CAPABILITY_DELETE_REFS as CAPABILITY_DELETE_REFS, CAPABILITY_INCLUDE_TAG as CAPABILITY_INCLUDE_TAG, CAPABILITY_MULTI_ACK as CAPABILITY_MULTI_ACK, CAPABILITY_MULTI_ACK_DETAILED as CAPABILITY_MULTI_ACK_DETAILED, CAPABILITY_NO_DONE as CAPABILITY_NO_DONE, CAPABILITY_NO_PROGRESS as CAPABILITY_NO_PROGRESS, CAPABILITY_OFS_DELTA as CAPABILITY_OFS_DELTA, CAPABILITY_QUIET as CAPABILITY_QUIET, CAPABILITY_REPORT_STATUS as CAPABILITY_REPORT_STATUS, CAPABILITY_SHALLOW as CAPABILITY_SHALLOW, CAPABILITY_SIDE_BAND_64K as CAPABILITY_SIDE_BAND_64K, CAPABILITY_THIN_PACK as CAPABILITY_THIN_PACK, COMMAND_DEEPEN as COMMAND_DEEPEN, COMMAND_DONE as COMMAND_DONE, COMMAND_HAVE as COMMAND_HAVE, COMMAND_SHALLOW as COMMAND_SHALLOW, COMMAND_UNSHALLOW as COMMAND_UNSHALLOW, COMMAND_WANT as COMMAND_WANT, MULTI_ACK as MULTI_ACK, MULTI_ACK_DETAILED as MULTI_ACK_DETAILED, Protocol as Protocol, ProtocolFile as ProtocolFile, ReceivableProtocol as ReceivableProtocol, SIDE_BAND_CHANNEL_DATA as SIDE_BAND_CHANNEL_DATA, SIDE_BAND_CHANNEL_FATAL as SIDE_BAND_CHANNEL_FATAL, SIDE_BAND_CHANNEL_PROGRESS as SIDE_BAND_CHANNEL_PROGRESS, SINGLE_ACK as SINGLE_ACK, TCP_GIT_PORT as TCP_GIT_PORT, ZERO_SHA as ZERO_SHA, ack_type as ack_type, capability_agent as capability_agent, extract_capabilities as extract_capabilities, extract_want_line_capabilities as extract_want_line_capabilities, symref_capabilities as symref_capabilities
from dulwich.refs import ANNOTATED_TAG_SUFFIX as ANNOTATED_TAG_SUFFIX, write_info_refs as write_info_refs
from dulwich.repo import BaseRepo as BaseRepo, Repo as Repo
from typing import Any, AnyStr, Callable, Dict, IO, Iterable, List, Optional, Set, Tuple, Type

logger: Logger

class Backend:
    def open_repository(self, path: AnyStr) -> BackendRepo: ...
...
class BackendRepo:
    ...
    def fetch_objects(self, determine_wants: Any, graph_walker: Any, progress: Callable, get_tagged: Optional[Callable] = ...) -> None: ...
...
class UploadPackHandler(PackHandler):
    repo: Any = ...
    advertise_refs: Any = ...
    def __init__(self, backend: Any, args: Any, proto: Any, stateless_rpc: Optional[Any] = ..., advertise_refs: bool = ...) -> None: ...
    @classmethod
...
def serve_command(handler_cls: Type[Handler], argv: List[str] = ..., backend: Optional[Type[Backend]] = ..., inf: IO = ..., outf: IO = ...) -> int: ...
