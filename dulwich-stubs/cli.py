...
class cmd_archive(Command):
	...
	def run(self, args):
		...
		parser.add_option("--remote", type=str, help="Retrieve archive from specified remote repo")
		options, args = parser.parse_args(args)
		committish = args.pop(0)
		if options.remote:
			client, path = get_transport_and_path(options.remote)
			client.archive(path, committish, sys.stdout.write, write_error=sys.stderr.write)
		else:
			porcelain.archive('.', committish, outstream=sys.stdout, errstream=sys.stderr)
...
class cmd_log(Command):
	...
	def run(self, args):
		...
		porcelain.log(
				".", paths=args, reverse=options.reverse, name_status=options.name_status, outstream=sys.stdout
				)
...
class cmd_daemon(Command):
	...
	def run(self, args):
		...
		parser = optparse.OptionParser()
		parser.add_option(
				"-l", "--listen_address", dest="listen_address", default="localhost", help="Binding IP address."
				)
	...
	def run(self, args):
		...
		if any(names for (kind, names) in status.staged.items()):
			...
			for kind, names in status.staged.items():
				for name in names:
					sys.stdout.write(f"\t{kind}: {name.decodesys.getfilesystemencoding}\n")
			sys.stdout.write("\n")
		if status.unstaged:
...
class cmd_ls_remote(Command):
	...
	def run(self, args):
		...
		for ref in sorted(refs):
			sys.stdout.write(f"{ref}\t{refsref}\n")
...
class cmd_ls_tree(Command):
	...
	def run(self, args):
		parser = optparse.OptionParser()
		parser.add_option("-r", "--recursive", action="store_true", help="Recusively list tree contents.")
		...
		try:
			...
			treeish = None
		porcelain.ls_tree(
				'.', treeish, outstream=sys.stdout, recursive=options.recursive, name_only=options.name_only
				)
