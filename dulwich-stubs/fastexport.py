...
from fastimport import errors as fastimport_errors  # noqa: E402
...
__all__ = ["split_email", "GitFastExporter", "GitImportProcessor"]
...
class GitFastExporter(object):
	...
	def _export_commit(self, commit, ref, base_tree=None):
		...
		cmd = commands.CommitCommand(
				ref,
				marker, (author, author_email, commit.author_time, commit.author_timezone),
...
class GitImportProcessor(processor.ImportProcessor):
	...
	def commit_handler(self, cmd):
		...
		commit.tree = commit_tree(
				self.repo.object_store, ((path, hexsha, mode) for (path, (mode, hexsha)) in self._contents.items())
				)
	...
	def _reset_base(self, commit_id):
		...
		if commit_id != ZERO_SHA:
			...
			for (path, mode, hexsha) in (self.repo.object_store.iter_tree_contents(tree_id)):
