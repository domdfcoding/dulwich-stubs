...
def unified_diff(
	...
	for group in SequenceMatcher(None, a, b).get_grouped_opcodes(n):
		if not started:
			...
			yield '--- {}{}{}'.format(fromfile.decode(tree_encoding), fromdate, lineterm).encode(output_encoding)
			yield '+++ {}{}{}'.format(tofile.decode(tree_encoding), todate, lineterm).encode(output_encoding)
...
def write_blob_diff(f, old_file, new_file):
	...
	f.writelines(
			gen_diff_header((old_path, new_path), (old_mode, new_mode),
...
def write_tree_diff(f, store, old_tree, new_tree, diff_binary=False):
	...
	for (oldpath, newpath), (oldmode, newmode), (oldsha, newsha) in changes:
		write_object_diff(
				f, store, (oldpath, oldmode, oldsha), (newpath, newmode, newsha), diff_binary=diff_binary
				)
